...
useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (!doktorId) {
      window.location.href = "/step3-tip-pregleda";
      return;
    }
  ...
  const validate = (): boolean => {
    ...
    if (isDoctorMode) {
      if (!odabraniPacijentId) errs.ime = "Morate odabrati pacijenta";
    } else {
    ...
    if (form.komentar.length > 255)
      errs.komentar = "Komentar ne smije imati više od 255 karaktera";
    setErrors(errs);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isDoctorMode && odabraniPacijentId) {
        formData.append("idPacijent", String(odabraniPacijentId));
      }
      ...
      if (!res.ok) {
        ...
        alert(err.poruka || err.message || "Greška pri kreiranju rezervacije.");
        return;
      }
      ...
      localStorage.setItem(
        "patientData",
        JSON.stringify({
          ime: isDoctorMode ? odabraniPacijent?.ime ?? "" : form.ime,
          prezime: isDoctorMode ? odabraniPacijent?.prezime ?? "" : form.prezime,
          email: isDoctorMode ? odabraniPacijent?.email ?? "" : form.email,
        })
      );
          ...
          {isDoctorMode && (
            <div className="mt-4 flex items-center gap-3 px-4 py-3 bg-blue-50 border border-blue-200 rounded-xl">
              ...
              <p className="text-sm text-blue-700">
                Rezervišete kao <strong>doktor</strong>. Odaberite pacijenta iz liste.
              </p>
            </div>
          )}
        </div>
          ...
          <div>
            {selectedTermin ? (
              <div className="bg-white rounded-xl shadow-md border border-gray-200 p-6 sticky top-6">
                ...
                {countdown !== null && (
                  <div className={`flex items-center justify-between px-4 py-3 rounded-lg mb-4 ${countdown <= 30 ? "bg-red-50 border border-red-200" : "bg-amber-50 border border-amber-200"}`}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  {isDoctorMode ? (
                    ...
                    <div>
                      ...
                      <select
                        value={odabraniPacijentId ?? ""}
                        onChange={(e) => {
                          setOdabraniPacijentId(Number(e.target.value));
                          setErrors((err) => ({ ...err, ime: undefined }));
                        }}
                        className={`w-full px-3 py-2 border rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 ${errors.ime ? "border-red-400 bg-red-50" : "border-gray-300"}`}
                      >
