...
export async function posaljiPodsjetnik(podaci: RezervacijaEmailPodaci): Promise<void> {
  const {
    pacijentEmail, pacijentIme, pacijentPrezime,
    doktorIme, doktorPrezime, doktorSpecijalizacija,
    datum, vrijeme, rezervacijaId,
  ...
  await resend.emails.send({
    ...
    to: pacijentEmail,
    subject: `⏰ Podsjetnik: Vaš termin je sutra #${rezervacijaId}`,
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
        <div style="background-color: #f59e0b; padding: 24px; text-align: center;">
          <h1 style="color: white; margin: 0; font-size: 22px;">⏰ Podsjetnik za termin</h1>
          <p style="color: #fef3c7; margin: 6px 0 0;">Rezervacija #${rezervacijaId}</p>
        </div>
        <div style="padding: 30px;">
          ...
          <table style="width: 100%; border-collapse: collapse; margin: 20px 0;">
            <tr style="background-color: #f8f9fa;">
              <td style="padding: 12px 16px; border: 1px solid #dee2e6; font-weight: bold;">Doktor</td>
              <td style="padding: 12px 16px; border: 1px solid #dee2e6;">Dr. ${doktorIme} ${doktorPrezime}</td>
            </tr>
            <tr>
              <td style="padding: 12px 16px; border: 1px solid #dee2e6; font-weight: bold;">Specijalizacija</td>
              <td style="padding: 12px 16px; border: 1px solid #dee2e6;">${doktorSpecijalizacija}</td>
            </tr>
            <tr style="background-color: #f8f9fa;">
              <td style="padding: 12px 16px; border: 1px solid #dee2e6; font-weight: bold;">Datum</td>
              <td style="padding: 12px 16px; border: 1px solid #dee2e6;">${datum.toLocaleDateString('bs-BA')}</td>
            </tr>
            <tr>
              <td style="padding: 12px 16px; border: 1px solid #dee2e6; font-weight: bold;">Vrijeme</td>
              <td style="padding: 12px 16px; border: 1px solid #dee2e6;">${formatVrijeme(vrijeme)} h</td>
            </tr>
          </table>
          <div style="background-color: #fef3c7; border: 1px solid #f59e0b; border-radius: 4px; padding: 14px 16px;">
            <strong>⚠️ Napomena:</strong> Molimo dođite <strong>10 minuta</strong> ranije. Ponesite ličnu kartu i zdravstvenu knjižicu.
          </div>
        </div>
        <div style="background-color: #f8f9fa; padding: 16px; text-align: center; border-top: 1px solid #dee2e6;">
  ...
  console.log(`⏰ Podsjetnik #${rezervacijaId} poslan na: ${pacijentEmail}`);
}