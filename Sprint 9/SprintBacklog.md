...
| US-38 | **Lista čekanja za termine** — Kao pacijent, želim se prijaviti na listu čekanja za popunjene dane, kako bih automatski dobio ponudu za slobodan termin u slučaju otkazivanja. | Amina Alispahić | Done | Kreiranje tabele `ListaCekanja`; event-driven servis sa TTL tajmerom od 2 min; automatska eskalacija na sljedećeg korisnika |
...
| US-41 | **Obavijest pacijentu o otkazanom/pomjerenom terminu** — Kao pacijent, želim primiti email i in-app obavijest o promjeni termina, kako bih mogao odmah prihvatiti ili odbiti novi ponuđeni slot. | Merjem Milišić | Done | Email šabloni sa dinamičkim linkovima; in-app login modal koji blokira interfejs; ažuriranje u `CONFIRMED` ili oslobađanje slota |
...
| US-43 | **Profil korisnika** — Kao korisnik, želim standardizovan prikaz profila sa zaštićenim email poljem i ispravnim prikazom datuma, kako bi moji podaci bili kozistentni i tačni. | Lamija Halilović | Done | Postavljanje `email` polja u read-only status |
